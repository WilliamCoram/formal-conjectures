...
public import FormalConjecturesTest.Leopoldts.ForMathlib.NumberTheory.NumberField.EmbeddingsBasis
...
$c \in \mathcal{O}_K$ (`PadicIwasawaLog.PadicComplex.norm_le_one_of_isIntegral`).
-/
theorem tendsto_map_of_forall_eventually_dvd {y : ℕ → 𝓞 K}
  ...
  obtain ⟨M, hM⟩ := exists_pow_lt_of_lt_one hε
    (PadicIwasawaLog.PadicComplex.norm_natCast_p_lt_one (p := p))
